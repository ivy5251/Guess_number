...
const generateAllPossibilities = (): string[] => {
  const result: string[] = [];
...
// Calculate A and B
const calculateAB = (secret: string, guess: string) => {
  let a = 0;
  let b = 0;
  for (let i = 0; i < 4; i++) {
    if (guess[i] === secret[i]) {
...
const Menu = ({ onSelectMode }: { onSelectMode: (mode: GameMode) => void }) => {
  return (
    <div className="flex flex-col items-center justify-center h-full p-6 space-y-8 max-w-md mx-auto w-full">
      <div className="text-center space-y-2">
        <h1 className="text-4xl font-bold text-indigo-600 tracking-tight">1A2B Master</h1>
        <p className="text-slate-500">The Classic Code-Breaking Game</p>
      </div>
      ...
      <div className="grid gap-4 w-full">
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-xl font-bold">Computer Guesses</h3>
              <p className="text-indigo-200 text-sm mt-1">You think of a number, Computer guesses instantly.</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-xl font-bold">Player Guesses</h3>
              <p className="text-slate-500 text-sm mt-1">Computer thinks of a number, you try to guess it.</p>
            </div>
      ...
      <div className="text-xs text-slate-400 text-center max-w-xs">
        <p>Rules: The secret number has 4 distinct digits (0-9).</p>
        <p>'A' = Correct digit, correct position.</p>
        <p>'B' = Correct digit, wrong position.</p>
      </div>
    </div>
  );
};
...
const AIGuessingGame = ({ onBack }: { onBack: () => void }) => {
  ...
  const [feedbackA, setFeedbackA] = useState<number>(0);
  const [feedbackB, setFeedbackB] = useState<number>(0);
  ...
  return (
    <div className="flex flex-col h-full max-w-2xl mx-auto w-full p-4">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <h2 className="text-lg font-bold text-indigo-700">Computer Guesses</h2>
        <div className="bg-indigo-50 text-indigo-700 px-3 py-1 rounded-full text-xs font-mono font-medium">
      ...
      <div className="flex-grow overflow-y-auto space-y-3 mb-6 pr-2 custom-scrollbar">
        {history.map((turn, idx) => (
          <div key={idx} className="flex items-center justify-between bg-white border border-slate-100 p-3 rounded-lg shadow-sm opacity-80">
            <span className="text-slate-400 font-mono text-sm">#{idx + 1}</span>
            <span className="font-mono text-xl tracking-widest text-slate-700">{turn.guess}</span>
            <span className={`font-bold font-mono ${turn.a === 4 ? 'text-green-600' : 'text-orange-500'}`}>
              {turn.a}A{turn.b}B
            </span>
          </div>
        ))}
      ...
      {/* Active Turn UI */}
      <div className="bg-white border-t border-slate-200 p-6 rounded-t-2xl shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
        {gameOver ? (
          <div className="text-center space-y-4">
            {error ? (
                <>
                    <div className="text-5xl">😵‍💫</div>
                    <h3 className="text-xl font-bold text-red-600">Contradiction!</h3>
                    <p className="text-slate-600 text-sm">{error}</p>
                </>
            ) : (
                <>
                    <div className="text-5xl animate-bounce">🎉</div>
                    <h3 className="text-2xl font-bold text-slate-800">I got it!</h3>
                    <p className="text-slate-600">It took me {history.length} attempts.</p>
                </>
            )}
            <button 
              onClick={onBack}
          ...
          <div className="space-y-6">
            <div className="text-center">
              <p className="text-xs uppercase tracking-wide text-slate-400 mb-2 font-bold">My Guess</p>
              <div className="text-6xl font-mono font-bold text-indigo-600 tracking-[0.2em] ml-4">
                {currentGuess}
              </div>
            </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4 sm:gap-8">
                  ...
                  <div className="space-y-2">
                    <label className="block text-xs font-bold text-center text-slate-500 uppercase">A (Correct Place)</label>
                  ...
                  {/* B Selector */}
                  <div className="space-y-2">
                    <label className="block text-xs font-bold text-center text-slate-500 uppercase">B (Wrong Place)</label>
                    <div className="flex justify-center gap-1 flex-wrap">
                      {[0, 1, 2, 3, 4].map(num => (
                        <button
                          key={`b-${num}`}
                ...
                {error && <div className="text-red-500 text-sm text-center animate-pulse font-medium">{error}</div>}
                ...
                >
                  <span>Confirm</span>
                  <span className="text-indigo-300 text-sm font-normal">(Instant)</span>
                </button>
              </div>
            )}
          </div>
...
const UserGuessingGame = ({ onBack }: { onBack: () => void }) => {
  ...
  return (
    <div className="flex flex-col h-full max-w-2xl mx-auto w-full p-4">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <h2 className="text-lg font-bold text-slate-800">Player Guesses</h2>
      ...
      <div className="flex-grow overflow-y-auto space-y-3 mb-6 pr-2 custom-scrollbar">
         {guesses.length === 0 && (
           <div className="text-center text-slate-400 mt-20">
             ...
             <p>I have a secret 4-digit number.</p>
             <p>Can you guess it?</p>
           </div>
         )}
        {guesses.map((turn, idx) => (
          <div key={idx} className="flex items-center justify-between bg-white border border-slate-200 p-4 rounded-lg shadow-sm">
            <span className="text-slate-400 font-mono text-sm">#{idx + 1}</span>
            <span className="font-mono text-xl tracking-widest text-slate-800">{turn.guess}</span>
            <span className={`font-bold font-mono ${turn.a === 4 ? 'text-green-600' : 'text-orange-500'}`}>
              {turn.a}A{turn.b}B
            </span>
          </div>
      ...
      <div className="bg-white border-t border-slate-200 p-6 rounded-t-2xl shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
        {won ? (
           <div className="text-center space-y-4">
            <div className="text-5xl animate-bounce">🏆</div>
            <h3 className="text-2xl font-bold text-slate-800">You Won!</h3>
            <p className="text-slate-600">The number was <span className="font-mono font-bold">{secret}</span></p>
            <button 
              onClick={onBack}
              className="bg-indigo-600 text-white px-8 py-3 rounded-full font-bold hover:bg-indigo-700 transition-colors"
            >
